    ...
    <div className="min-h-screen bg-[#F3F3F1] text-[#000000] py-12 px-6 flex flex-col">
      ...
      <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row gap-4">
        ...
        <input
          ...
          className="w-32 px-4 py-2 border rounded shadow focus:outline-none"
        />
        <input
          type="number"
      ...
      <div className="max-w-7xl mx-auto flex-grow">
        {filtered.length === 0 ? (
          <p className="text-center text-gray-600 text-lg">No hay productos que coincidan.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filtered.map(product => {
              ...
              const finalPrice = hasDiscount
                ...
                : product.price;
              return (
                <div key={product.id} className="flex flex-col justify-end p-8 rounded-xl bg-[#F3F3F1] transition hover:shadow-xl">
                  ...
                  <div className="mb-4 flex items-center">
                    {hasDiscount ? (
                      <>
                        ...
                        <span className="bg-[#8F5C0C] text-white px-2 py-1 rounded-full text-xs">{product.discount_percent}% OFF</span>
                      </>
                    ) : (
                      <span className="text-2xl font-bold text-[#8F5C0C]">${product.price.toFixed(2)}</span>
                    )}
                  </div>
                  <p className="mb-6 text-[#7C5F42]">{product.description}</p>
                  <div className="flex flex-col items-start space-y-2">
                    {product.stock > 0 ? (
                      <>
                        <button
                          onClick={() => handleAddToCart(product.id)}
                          className="px-6 py-3 rounded-full font-semibold uppercase bg-[#8F5C0C] text-white hover:bg-opacity-90 w-full"
                        >
                          Add to Cart
                        </button>
                        <p className="text-sm text-green-600 w-full text-center">{product.stock} disponibles</p>
                      </>
                    ) : (
                      <p className="text-sm text-red-600 w-full text-center">Agotado</p>
                    )}
                  </div>
                </div>
              );
        ...
        <div className="max-w-7xl mx-auto mt-8">
          <div className="flex justify-center space-x-4">
            <button
              ...
              className="px-5 py-2 rounded-full font-semibold transition-colors disabled:opacity-50 bg-amber-600 text-white hover:bg-amber-500"
            >
              <ChevronLeft size={18} /> Anterior
            </button>
            <span className="px-4 py-2 rounded-full font-semibold bg-gray-100 text-gray-900">
